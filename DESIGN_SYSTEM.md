...
### Scrollbar (the `--bs-*` tokens)
The native browser scrollbar is recolored to the brand on **every** surface —
there is no grey default. `--bs-thumb` (`--color-border-bright` rest) →
`--bs-thumb-hover` (`--primary-blue`), `--bs-size` 12px, `--bs-radius` **3px**
(house radius, not Millis's sharp 2px), slimmed by a 3px transparent inset
border + padding-box clip. The optional animated overlay rail reads
`--bs-accent` (vertical brand gradient), `--bs-track`, `--bs-mono`, and the
`--bs-readout-*` pair. Two layers, documented at §5 / `/design-system` §22.
...
| `Footer` | Nav/Products/Connect mono columns (plain links — all three columns consistent, no letter roll), stroked `QUBETX` wordmark (`RevealText mode="chars"`), back-to-top with FM `pathLength` progress ring → `lenis.scrollTo(0)`, `SysStatus` line, Konami hint glyph (40% opacity). |
...
| `About` | "Detail is the product." → `RoutedText` lead paragraph (Pretext obstacle routing around the cube) + Pretext paragraph → `StatValue` cells (count-up entrance + re-verify hover) → process striped rows. |
...
| `StatValue` | About stat. Count-up entrance on first view (0 → value in exponential-approach slot-roll steps — big jumps when far, single ticks near the end — quiet intermediates, arrival-blue landing); hover re-verify = brisk count down to 0, then the same climb back + label `decode()` (decode stays on the LABEL node — never the same node as the roll). The displayed number always converges on a goal (0 or the value), so pointer-leave mid-count just turns the count around — no stuck/snapped states. Slot direction mirrors numeric direction (up rolls up, down rolls down). Prefix/digits/suffix parsing preserved (`"100%"` counts as 3 zero-padded digits + `%`); reduced motion shows the server-rendered final value untouched. |
...
| `Modal` | Controlled dialog primitive (`open, onClose, title, eyebrow?, actions?`). Body-portal past transformed ancestors (gotcha #14), FM `AnimatePresence`, focus trap + Esc + scrim dismiss + focus return, body scroll-lock with `lenis.stop()/start()`. Centered card → bottom sheet <600px. Reduced motion = instant. The general overlay; `MobileMenu` is the nav specialization. |
...
| `ScrollTrace` | Scroll-scrubbed circuit trace + isometric cube junctions (`scrollTracePath.ts`, pure). Paused timeline of `svg.createDrawable` segments **seek()ed from Lenis progress** (draws down, reverses up; native-scroll fallback). Gutter = true outer margin when `(vw − --container-max)/2 > 88`, else hairline lane at x≈11. Hidden <1024px. No SVG filters ever (glow = second wider stroke at 0.06). **Ships in the kit as an OPTIONAL expressive decoration** — `sectionIds` prop places the junctions; the specific design (path, junction glyphs, gutter) may vary per surface within the brand language (see §21 on /design-system). |
...
`brandScrollbar`/`useBrandScrollbar` (the opt-in overlay scrollbar — a brand-gradient kinetic rule + optional `[data-bs-section]` ticks + a mono `SEC NN · NN%` readout, over a hidden native bar; anime via the seam, reduced motion static, re-measure via `resizeCoordinator`, no plumb-bob).
...
| Letter roll | stacked copies, y −100%, 420ms `out(4)`, stagger 18ms | `RollingLabel` (link lists — kit component; no longer on the site footer) |
...
| Stat count-up | 0 → value in exponential-approach quiet steps (~190ms cadence, single ticks at the end) → arrival-blue landing | About stats, first view |
| Re-verify | brisk count down to 0 (~140ms cadence), then the climb back to the same value + label decode on hover; pointer-leave mid-count turns the count around | About stats |
