...
| `Services` | Grid cells of `ServiceCard` (pill index, lucide icon nudge, proximity glow via `useProximityGlow` + `data-glow`). Card ids `service-{id}` are the dropdown anchors. |
